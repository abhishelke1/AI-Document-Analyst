    ...
    if st.session_state.vector_db is None:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking (Hugging Face API)..."):
                ...
                # --- FIX START: AUTHENTICATION HANDLING ---
                try:
                    # 1. Fetch Token from Secrets
                    hf_token = st.secrets["HUGGINGFACEHUB_API_TOKEN"]
                    
                    # 2. FORCE the environment variable (Crucial for LangChain/HuggingFaceHub)
                    ...
                    st.error("Token missing. Add 'HUGGINGFACEHUB_API_TOKEN' to secrets.")
                    st.stop()
                
                # 3. Use a Non-Gated Model (Safer for testing)
                # We use Mistral here because it DOES NOT require email approval like Llama-3
